...
parser.add_argument('--cut_ratio', default=0.1, type=float)
...
out_point_path_norm = os.path.join(out_dir,site_name+'_xyz_rgb_{}_norm_1car_{}_cut{}.pcd'.format(sample_ratio,car_sample_ratio, cut_ratio))
...
out_car_path_norm = os.path.join(out_dir, '{}_{}_cut{}.pcd'.format(car_name, car_sample_ratio, cut_ratio))
...
#R_elev = np.zeros((3,3))
#R_azim[0, 0]=1
#R_azim[1, 1]=np.cos(np.deg2rad(azim))
#R_azim[2, 2]=np.cos(np.deg2rad(azim))
#R_azim[1, 2]=-np.sin(np.deg2rad(azim))
#R_azim[2, 1]=np.sin(np.deg2rad(azim))
#R_elev[1, 1]=1
#R_elev[0, 0]=np.cos(np.deg2rad(elev))
#R_elev[2, 2]=np.cos(np.deg2rad(elev))
#R_elev[0, 2]=np.sin(np.deg2rad(elev))
#R_elev[2, 0]=-np.sin(np.deg2rad(elev))
#R = np.matmul(R_elev, R_azim)
...
x_max = benz_norm[:, 1].max(axis=0)
x_min = benz_norm[:, 1].min(axis=0)
benz_norm_cut = benz_norm[benz_norm[:,1] > x_min + (x_max-x_min)*cut_ratio]
#print mean_benz
#print np.max(benz_norm[:])
#print benz_norm[0, ...]
car_rgb = [args.car_r, args.car_g, args.car_b]

car_sample_points = benz_norm_cut[::car_sample_ratio]
#car_sample_points = np.transpose(np.matmul(R, np.transpose(car_sample_points)))
...
with open(out_point_path_norm, 'w') as of:
    ...
    of.write('TYPE F F F F\r\n')
    of.write('COUNT 1 1 1 1\r\n')
    of.write('WIDTH {}\r\n'.format(len(sample_points)+car_sample_points.shape[0]))
    of.write('HEIGHT 1\r\n')
    of.write('VIEWPOINT 0 0 0 1 0 0 0\r\n')
    of.write('POINTS {}\r\n'.format(len(sample_points)+car_sample_points.shape[0]))
    of.write('DATA ascii\r\n')
