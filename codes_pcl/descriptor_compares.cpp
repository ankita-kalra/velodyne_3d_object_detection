...
std::string used_descriptor="fpfh";
void
showHelp(char *filename)
{
	...
	std::cout << "***************************************************************************" << std::endl;
	std::cout << "*                                                                         *" << std::endl;
	std::cout << "*             Object Detection Wild          *" << std::endl;
	std::cout << "*                                                                         *" << std::endl;
	...
	std::cout << "     -desc:                  Type of Descriptor" << std::endl;
...
{    
	cout << "inside parse command line" << endl;
	//Show help
	...
	if (pcl::console::parse_argument(argc, argv, "--sampling_algorithm", used_sampling_algorithm) != -1)
	{
	...
	cout << pcl::console::parse_argument(argc, argv, "--desc", used_descriptor) << endl;
	
	if (pcl::console::parse_argument(argc, argv, "--desc", used_descriptor) != -1)
	{
		if (true)
		{   
			cout << used_descriptor << endl;
			used_descriptor = used_descriptor;
		}
		else
		{
			std::cout << "Wrong algorithm name.\n";
...
main(int argc, char *argv[])
{
	...
	cout << "used _ descriptor" << used_descriptor << endl;
	norm_est.setSearchMethod(kdtree);
	norm_est.compute(*model_normals);
	if (used_descriptor == "shot")
	{
	...
	else if (used_descriptor.compare("pfh") == 0)
	{
		// PFH estimation object.
		pcl::PFHEstimation<pcl::PointXYZRGB, pcl::Normal, pcl::PFHSignature125> descr_est;
		descr_est.setInputCloud(model_keypoints);
		descr_est.setInputNormals(model_normals);
		...
		pcl::PointCloud<pcl::PFHSignature125>::Ptr model_descriptors1(new pcl::PointCloud<pcl::PFHSignature125>());
		pcl::PointCloud<pcl::PFHSignature125>::Ptr scene_descriptors1(new pcl::PointCloud<pcl::PFHSignature125>());
		descr_est.compute(*model_descriptors1);

		descr_est.setInputCloud(scene_keypoints);
		descr_est.setInputNormals(scene_normals);
		descr_est.setSearchSurface(scene);
		descr_est.compute(*scene_descriptors1);
		cout << "here 2" << endl;
	}
	else if (used_descriptor.compare("vpfh") == 0)
	{
		pcl::VFHEstimation<pcl::PointXYZRGB, pcl::Normal, pcl::VFHSignature308> descr_est;
		descr_est.setInputCloud(model_keypoints);
		descr_est.setInputNormals(model_normals);
		descr_est.setSearchMethod(kdtree);
		...
		descr_est.setNormalizeDistance(false);
		...
		descr_est.setSearchSurface(model);
		descr_est.compute(*model_descriptors);
		cout << model_descriptors->size() << endl;
